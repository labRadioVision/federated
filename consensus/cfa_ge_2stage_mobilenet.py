                ...
                if epoch == 0:
                    ...
                    for neighbor_index in range(self.neighbor_vec.size):
                        ...
                        [W_up_l1, n_up_l1, W_up_l2, n_up_l2] = self.federated_weights_computing2(
                            ...
                            'temp_datamat{}_{}.mat'.format(self.ii_saved_local, epoch), self.ii_saved_local, self.neighbor_vec[neighbor_index],
                    ...
                    for neighbor_index in range(self.neighbor_vec.size):
                        while not os.path.isfile(
                                'datamat{}_{}.mat'.format(self.neighbor_vec[neighbor_index], epoch)):
                            # print('Waiting for datamat{}_{}.mat'.format(ii_saved_local - 1, epoch))
                            pause(1)
                        try:
                            mathcontent = sio.loadmat('datamat{}_{}.mat'.format(self.neighbor_vec[neighbor_index], epoch))
                        ...
                        with tf.Session() as sess3:
                            ...
                            if self.ML_model == 1:
                                g_W_c_vect_l1[:, :, :, self.neighbor_vec[neighbor_index]] = g_W_c_l1
                                g_b_c_vect_l1[:, self.neighbor_vec[neighbor_index]] = g_b_c_l1
                                g_W_c_vect_l2[:, :, self.neighbor_vec[neighbor_index]] = g_W_c_l2
                                g_b_c_vect_l2[:, self.neighbor_vec[neighbor_index]] = g_b_c_l2
                            elif self.ML_model == 2:
                                g_W_c_vect_l1[:, :, self.neighbor_vec[neighbor_index]] = g_W_c_l1
                                g_b_c_vect_l1[:, self.neighbor_vec[neighbor_index]] = g_b_c_l1
                                g_W_c_vect_l2[:, :, self.neighbor_vec[neighbor_index]] = g_W_c_l2
                    ...
                    for neighbor_index in range(self.neighbor_vec.size):
                        while not os.path.isfile(
                                'datagrad{}_{}.mat'.format(self.neighbor_vec[neighbor_index], epoch)):
                            pause(1)
                        try:
                            mathcontent = sio.loadmat('datagrad{}_{}.mat'.format(self.neighbor_vec[neighbor_index], epoch))
                        except:
                            pause(3)
                            mathcontent = sio.loadmat('datagrad{}_{}.mat'.format(self.neighbor_vec[neighbor_index], epoch))
                        ...
                        try:
                            ...
                            pause(5)
                            print('datagrad{}_{}.mat'.format(self.neighbor_vec[neighbor_index], epoch))
                            del mathcontent
                            mathcontent = sio.loadmat('datagrad{}_{}.mat'.format(self.neighbor_vec[neighbor_index], epoch))
                            gradW_up_neigh_l1 = np.asarray(mathcontent['grad_weights1'])
                            gradW_up_neigh_l2 = np.asarray(mathcontent['grad_weights2'])
        ...
        if (self.federated):
            if self.devices > 1:
                if epoch == 0:
                    ...
                    for neighbor_index in range(self.neighbor_vec.size):
                        while not os.path.isfile(
                                'datagrad{}_{}.mat'.format(self.neighbor_vec[neighbor_index], epoch - 1)):
                            pause(1)
                        try:
                            mathcontent = sio.loadmat(
                                'datagrad{}_{}.mat'.format(self.neighbor_vec[neighbor_index], epoch - 1))
