...
def getFederatedWeight_gradients(n_W_l1, n_W_l2, n_b_l1, n_b_l2, federated, devices, ii_saved_local, epoch, v_loss,eng, x_train2, y_train2, neighbors, W_l1_saved, W_l2_saved, n_l1_saved, n_l2_saved):
    x_c = tf.placeholder(tf.float32, [None, inputs])  # 512 point FFT range measurements
    y_c = tf.placeholder(tf.float32, [None, classes])  # 0-7 HR distances => 8 classes

    W_ext_c_l1 = tf.placeholder(tf.float32, [filter, 1, number])
    b_ext_c_l1 = tf.placeholder(tf.float32, [number])

    W_ext_c_l2 = tf.placeholder(tf.float32, [multip*number, classes])
    b_ext_c_l2 = tf.placeholder(tf.float32, [classes])
    ...
    if (federated):
        if devices > 1:
            if epoch == 0:
                ...
                g_W_c_vect_l2 = np.zeros([multip*number, classes, devices])
                g_b_c_vect_l2 = np.zeros([classes, devices])
...
def processData(samples, iii, federated, tot_devices,fraction_training, neighbors_number,EPOCH_THRESHOLD):
    ...
    x_train = x_train.reshape(60000, inputs).astype('float32') / 255
    x_train2 = x_train[iii * samples:((iii + 1) * samples - 1), :]
    # x_test = x_test.reshape(10000, 784).astype('float32') / 255
    y_train_t = to_categorical(y_train)
    y_train2 = to_categorical(y_train[iii * samples:((iii + 1) * samples - 1)])
    ...
    # tf Graph Input
    x = tf.placeholder(tf.float32, [None, inputs])  # 512 POINT FFT RANGE MEASUREMENTS
...
#    Start training
    with tf.Session() as sess:
        ...
        for epoch in range(training_epochs):
            ...
            # W_val_l1, b_val_l1, W_val_l2, b_val_l2 = getFederatedWeight(n_W_l1, n_W_l2, n_b_l1, n_b_l2, federated, tot_devices, iii, epoch, val_loss, eng, neighbors_number)
            ##################################################
            ...
            if epoch < EPOCH_THRESHOLD:
                if epoch < 2:
                    ...
                    W_l2_saved = np.zeros((multip * number, classes, neighbors_number))
                    n_l1_saved = np.zeros((number, neighbors_number))
                    n_l2_saved = np.zeros((classes, neighbors_number))
...
if __name__ == "__main__":
    ...
    validation_train = 60000 # VALIDATION DATASET
    ###################################################################################
