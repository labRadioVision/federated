...
class CFA_ge_process:
    ...
    def getFederatedWeight_gradients_fast(self, n_W_l1, n_W_l2, n_b_l1, n_b_l2, federated, devices, ii_saved_local, epoch,
                                          v_loss, eng, x_train2, y_train2, neighbors, W_l1_saved, W_l2_saved,
                                          n_l1_saved, n_l2_saved):
        x_c = tf.placeholder(tf.float32, [None, 512])  # 512 point FFT range measurements
        y_c = tf.placeholder(tf.float32, [None, 8])  # 0-7 HR distances => 8 classes
        ...
        if (federated):
            if devices > 1:
                if epoch == 0:
                    ...
                    if epoch >= 9:
                        fileList_grad = glob.glob('datagrad{}_{}.mat'.format(ii_saved_local, epoch - 8),
                                                  recursive=False)
