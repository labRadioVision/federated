...
class Target_Server:
    ...
    def __init__(self, devices, model_target_parameters, federated=True, graph=0):
        self.federated = federated # true for federation active
        self.devices = devices # number of devices
        self.model_target_parameters_init = model_target_parameters
    ...
    def federated_target_weights_aggregation(self, epoch=0):
        ...
        # opportunistic best device
        print(self.running_reward)
        best_device = np.argmax(self.running_reward)
        print("Using model {} as target with runniing reward {}".format(best_device, self.running_reward[best_device]))
        while not os.path.isfile(self.outfile_models[best_device]):
            # implementing consensus
            print("waiting")
            pause(1)

        try:
            model_weights = np.load(self.outfile_models[best_device], allow_pickle=True)
        except:
            pause(5)
        ...
        if not stop_aggregation:
            for q in range(self.layers):
                self.model_target_parameters[q] = model_weights[q]

        # target model averaging
        # old_weights = self.model_target_parameters
        # self.model_target_parameters = self.model_target_parameters_init # reinitialize
        # for k in range(self.devices):
        ...
        #
        #     try:
        #         model_weights = np.load(self.outfile_models[k], allow_pickle=True)
        #     except:
        #         pause(5)
        ...
        #         combined_models += 1
        #         for q in range(self.layers):
        #             self.model_target_parameters[q] = self.model_target_parameters[q] + model_weights[q]
        #
        # if combined_models > 0: # normalize wrt the received models on server
        #     print("Received models to combine {}".format(combined_models))
        #     for q in range(self.layers):
        #         self.model_target_parameters[q] = self.model_target_parameters[q] * (1/combined_models)
        # else:
        #     self.model_target_parameters = old_weights
