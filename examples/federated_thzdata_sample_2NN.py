...
def getFederatedWeight_gradients(n_W_l1, n_W_l2, n_b_l1, n_b_l2, federated, devices, ii_saved_local, epoch, v_loss,eng, x_train2, y_train2, neighbors, W_l1_saved, W_l2_saved, n_l1_saved, n_l2_saved):
    x_c = tf.placeholder(tf.float32, [None, input_size])  # 512 point FFT range measurements
    y_c = tf.placeholder(tf.float32, [None, 8])  # 0-7 HR distances => 8 classes

    W_ext_c_l1 = tf.placeholder(tf.float32, [input_size, intermediate_nodes])
    b_ext_c_l1 = tf.placeholder(tf.float32, [intermediate_nodes])

    W_ext_c_l2 = tf.placeholder(tf.float32, [intermediate_nodes, 8])
    ...
    if (federated):
        if devices > 1:
            if epoch == 0:
                ...
                g_W_c_vect_l1 = np.zeros([input_size, intermediate_nodes, devices])
                g_b_c_vect_l1 = np.zeros([intermediate_nodes, devices])
                g_W_c_vect_l2 = np.zeros([intermediate_nodes, 8, devices])
        ...
        for epoch in range(training_epochs):
            ...
            if epoch < EPOCH_THRESHOLD:
                ...
                W_val_l1, b_val_l1, W_val_l2, b_val_l2, W_l1_saved, W_l2_saved, n_l1_saved, n_l2_saved = getFederatedWeight_gradients_fast(n_W_l1, n_W_l2, n_b_l1, n_b_l2, federated, tot_devices, iii,
                    epoch, val_loss, eng, x_train2, y_train2, neighbors_number, W_l1_saved, W_l2_saved, n_l1_saved, n_l2_saved)  # method with gradients exchange and two stage negotiation
            ###########################################################
...
if __name__ == "__main__":
    ...
    ii_saved = 0
    EPOCH_THRESHOLD = 4 # STARTING EPOCH FOR CFA-GE (2-STAGE NEGOTIATION), should be greather than 3
