...
parser.add_argument('-T', default=8, help="sets the number of training epochs", type=int)
...
def getFederatedWeight_gradients(n_W_l1, n_W_l2, n_b_l1, n_b_l2, federated, devices, ii_saved_local, epoch, v_loss,eng, x_train2, y_train2, neighbors, W_l1_saved, W_l2_saved, n_l1_saved, n_l2_saved):
    x_c = tf.placeholder(tf.float32, [None, input_data])  # 512 point FFT range measurements
    y_c = tf.placeholder(tf.float32, [None, classes])  # 0-7 HR distances => 8 classes

    W_ext_c_l1 = tf.placeholder(tf.float32, [input_data, intermediate_nodes])
    b_ext_c_l1 = tf.placeholder(tf.float32, [intermediate_nodes])

    W_ext_c_l2 = tf.placeholder(tf.float32, [intermediate_nodes, classes])
    ...
    if (federated):
        if devices > 1:
            if epoch == 0:
                ...
                g_W_c_vect_l1 = np.zeros([input_data, intermediate_nodes, devices])
                g_b_c_vect_l1 = np.zeros([intermediate_nodes, devices])
                g_W_c_vect_l2 = np.zeros([intermediate_nodes, classes, devices])
