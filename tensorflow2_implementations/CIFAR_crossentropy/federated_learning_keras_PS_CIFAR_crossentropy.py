...
parser.add_argument('-target', default=1.2, help="sets the target crossentropy loss to stop federation", type=float)
...
def create_q_model():
    ...
    if condition == 0:
        ...
        classification = layers.Dense(n_outputs, activation="softmax")(layer7)
...
def processParameterServer(devices, active_devices_per_round, federated, refresh_server=1):
    ...
    while True:
        ...
        aa = parameter_server.federated_target_weights_aggregation(epoch=0, aggregation_type=0)
        np.save(global_target_model, aa)
